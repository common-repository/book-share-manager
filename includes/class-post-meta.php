...
class class_bsm_post_meta{
	
	public function __construct(){
		add_action('add_meta_boxes', array($this, 'meta_boxes_bsm_sharer'));
		add_action('save_post', array($this, 'meta_boxes_bsm_sharer_save'));
	}

	public function bsm_sharer_meta_options($options = array()){
			
			$options['BSM Info'] = array(
		
								'bsm_user_id'=>array(
									'css_class'=>'bsm_user_id',					
									'title'=>'Sharer ID',
									'option_details'=>'Sharer ID / User ID',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),
									
								'bsm_user_pin'=>array(
									'css_class'=>'bsm_user_pin',					
									'title'=>'Sharer Pin',
									'option_details'=>'Sharer Pin / Secret number',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),
									
								'bsm_sharer_dept'=>array(
									'css_class'=>'bsm_sharer_dept',					
									'title'=>'Sharer Department',
									'option_details'=>'Sharer Department Name',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),
								
								'bsm_sharer_email'=>array(
									'css_class'=>'bsm_sharer_email',					
									'title'=>'Sharer Email',
									'option_details'=>'Sharer Email Address',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),
								
								'bsm_sharer_phone'=>array(
									'css_class'=>'bsm_sharer_phone',					
									'title'=>'Sharer Phone',
									'option_details'=>'Sharer Phone Number',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),
			...
			$options = apply_filters( 'bsm_filters_meta_options', $options );
	...
	public function bsm_sharer_meta_options_form(){
		
			global $post;
			
			$bsm_sharer_meta_options = $this->bsm_sharer_meta_options();
			...
			$i=1;
			foreach($bsm_sharer_meta_options as $key=>$options){
			if($i==1){
...
<script>
	...
	public function meta_boxes_bsm_sharer($post_type) {
			$post_types = array('bsm_sharer');
	 
			//limit meta box to certain post types
			if (in_array($post_type, $post_types)) 
			{
				add_meta_box('bsm_sharer_metabox',
					__('Saved Sharer Data','bsm'),
					array($this, 'bsm_sharer_meta_box_function'),
					$post_type,
					'normal',
					'high');
			}
		}
	public function bsm_sharer_meta_box_function($post) {
 
        // Add an nonce field so we can check for it later.
        wp_nonce_field('bsm_sharer_nonce_check', 'bsm_sharer_nonce_check_value');
       ...
       // $job_bm_report_data = get_post_meta($post -> ID, 'job_bm_report_data', true);
		...
		//var_dump($job_meta_options);
		foreach($bsm_sharer_meta_options as $options_tab=>$options){
        ...
        // Display the form, using the current value.
			...
			echo $this->bsm_sharer_meta_options_form(); 
			?>
			</div>
			<?php
   		}
	...
	public function meta_boxes_bsm_sharer_save($post_id){
			...
			if (!isset($_POST['bsm_sharer_nonce_check_value']))
				return $post_id;
	 
			$nonce = $_POST['bsm_sharer_nonce_check_value'];
	 
			// Verify that the nonce is valid.
			if (!wp_verify_nonce($nonce, 'bsm_sharer_nonce_check'))
				return $post_id;
