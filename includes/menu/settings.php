...
class class_job_bm_report_settings_page  {
	...
	public function job_bm_report_settings_options($options = array()){


			$class_bsm_functions = new class_bsm_functions();
			
			$job_bm_report_list_user_role = $class_bsm_functions->job_bm_report_list_user_role();




			$options['Options'] = array(
								'job_bm_report_list_per_page'=>array(
								...
								'job_bm_report_listing_duration'=>array(
			...
			$options['Job Post'] = array(
								'job_bm_report_account_required'=>array(
								...
								'job_bm_report_employer_account_role'=>array(
									...
									'input_args'=> $job_bm_report_list_user_role,
									),									
									
								'job_bm_report_applicant_account_role'=>array(
								...
								'job_bm_report_salary_currency'=>array(
			...
			$options['Style'] = array(
								'job_bm_report_featured_bg_color'=>array(
								...
								'job_bm_report_job_type_bg_color'=>array(
									...
									'input_values'=> '', // could be array
									'input_args'=> apply_filters('job_bm_report_filters_job_type_bg_color', array('freelance'=>'#46a6ff','full-time'=>'#2ec274','internship'=>'#a066ff','part-time'=>'#ffc24d','temporary'=>'#ff5741'))
									),
									
								'job_bm_report_job_status_bg_color'=>array(
									'css_class'=>'job_status_bg_color',					
									'title'=>'Job Status Background Color ?',
									'option_details'=>'Job Status Background Color',						
									'input_type'=>'text-multi', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									'input_args'=> apply_filters('job_bm_report_filters_job_status_bg_color', array('open'=>'#3ac170','closed'=>'#fa3218','filled'=>'#49a2ed','re-open'=>'#2fc2f9'))
									),								
			...
			$options = apply_filters( 'job_bm_report_settings_options', $options );
	...
	public function job_bm_report_settings_options_form(){
		
			global $post;
			
			$job_bm_report_settings_options = $this->job_bm_report_settings_options();
			...
			$i=1;
			foreach($job_bm_report_settings_options as $key=>$options){
			if($i==1){
...
new class_job_bm_report_settings_page();
...
if(empty($_POST['job_bm_report_hidden']))
	{


		$class_job_bm_report_settings_page = new class_job_bm_report_settings_page();
		
			$job_bm_report_settings_options = $class_job_bm_report_settings_page->job_bm_report_settings_options();
			
			foreach($job_bm_report_settings_options as $options_tab=>$options){
	...
	{	
		if($_POST['job_bm_report_hidden'] == 'Y') {
			//Form data sent
			...
			?>
			<div class="updated"><p><strong><?php _e('Changes Saved.', 'bsm' ); ?></strong></p></div>
...
<div class="wrap">
	...
	<div id="icon-tools" class="icon32"><br></div><?php echo "<h2>".__(bsm_plugin_name.' Settings', 'bsm')."</h2>";?>
		<form  method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
	<input type="hidden" name="job_bm_report_hidden" value="Y">
        <?php settings_fields( 'job_bm_report_plugin_options' );
				do_settings_sections( 'job_bm_report_plugin_options' );
			
			
	$class_job_bm_report_settings_page = new class_job_bm_report_settings_page();
        echo $class_job_bm_report_settings_page->job_bm_report_settings_options_form(); 
...
<p class="submit">
                    <input class="button button-primary" type="submit" name="Submit" value="<?php _e('Save Changes','bsm' ); ?>" />
                </p>
		</form>
