...
class class_bsm_post_meta_book{
	
	public function __construct(){
		add_action('add_meta_boxes', array($this, 'meta_boxes_bsm_book'));
		add_action('save_post', array($this, 'meta_boxes_bsm_book_save'));
	}

	public function bsm_book_meta_options($options = array()){
			
			$options['Book Info'] = array(
								...
								'bsm_book_category'=>array(
									'css_class'=>'bsm_book_category',					
									'title'=>'Book Category',
									'option_details'=>'Book category Name',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),
								
								'bsm_book_author'=>array(
									'css_class'=>'bsm_book_author',					
									'title'=>'Book Author',
									'option_details'=>'Book Author Name',						
									'input_type'=>'text', // text, radio, checkbox, select, 
									'input_values'=> '', // could be array
									),								
	...
	public function bsm_book_meta_options_form(){
		
			global $post;
			
			$bsm_book_meta_options = $this->bsm_book_meta_options();
			$html = '';
			...
			$i=1;
			foreach($bsm_book_meta_options as $key=>$options){
			if($i==1){
...
<script>
	...
	public function meta_boxes_bsm_book($post_type) {
			$post_types = array('bsm_book');
	 
			//limit meta box to certain post types
			if (in_array($post_type, $post_types)) 
			{
				add_meta_box('bsm_book_metabox',
					__('Saved Book Data','bsm'),
					array($this, 'bsm_book_meta_box_function'),
					$post_type,
					'normal',
					'high');
			}
		}
	public function bsm_book_meta_box_function($post) {
 
        // Add an nonce field so we can check for it later.
        wp_nonce_field('bsm_book_nonce_check', 'bsm_book_nonce_check_value');
       ...
       // $job_bm_report_data = get_post_meta($post -> ID, 'job_bm_report_data', true);
		...
		foreach($bsm_book_meta_options as $options_tab=>$options){
        ...
        // Display the form, using the current value.
			...
			echo $this->bsm_book_meta_options_form(); 
			?>
			</div>
			<?php
   		}
	...
	public function meta_boxes_bsm_book_save($post_id){
			...
			if (!isset($_POST['bsm_book_nonce_check_value']))
				return $post_id;
	 
			$nonce = $_POST['bsm_book_nonce_check_value'];
	 
			// Verify that the nonce is valid.
			if (!wp_verify_nonce($nonce, 'bsm_book_nonce_check'))
				return $post_id;
